...
void tb6612fng_speed(tb6612fng_t* tbf ,uint16_t speed,float sideA,float sideB){
	...
	__HAL_TIM_SET_COMPARE(tbf->htim,tbf->front_left,speed * sideA);
	__HAL_TIM_SET_COMPARE(tbf->htim,tbf->front_right,speed * sideB);
	__HAL_TIM_SET_COMPARE(tbf->htim,tbf->rear_left,speed * sideA);
	__HAL_TIM_SET_COMPARE(tbf->htim,tbf->rear_right,speed * sideB);

	uint16_t difference = 0;

	if(speed < last_speed){
		difference = last_speed - speed;
	}

	if(speed == 0 || (difference > 500)){
		HAL_GPIO_WritePin(tbf->stdby_front_port, tbf->stdby_front_pin, GPIO_PIN_RESET);
		HAL_GPIO_WritePin(tbf->stdby_rear_port, tbf->stdby_rear_pin, GPIO_PIN_RESET);
	}
	else{
		HAL_GPIO_WritePin(tbf->stdby_front_port, tbf->stdby_front_pin, GPIO_PIN_SET);
		HAL_GPIO_WritePin(tbf->stdby_rear_port, tbf->stdby_rear_pin, GPIO_PIN_SET);
